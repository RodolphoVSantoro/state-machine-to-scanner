...
void StringVec_push(StringVec *vec, char *str) {
    ...
    if (vec->data == NULL) {
        printf("Error increasing vec to %d\n", new_capacity);
        exit(1);
    }
...
char *StringVec_pop(StringVec *vec) {
    if (vec->size == 0) {
        printf("Error popping empty vec\n");
        exit(1);
    }
    vec->size--;
...
char *StringVec_get(StringVec *vec, int index) {
    if (index >= vec->size || index < 0) {
        printf("Error getting index %d from string vec of size %d\n", index, vec->size);
        exit(1);
    }
...
void StringVec_set(StringVec *vec, int index, char *str) {
    if (index >= vec->size || index < 0) {
        printf("Error setting index %d from string vec of size %d\n", index, vec->size);
        exit(1);
    }
    vec->data[index] = str;
}
...
void CharVec_push(CharVec *vec, char c) {
    ...
    if (vec->data == NULL) {
        printf("Error increasing vec from %d to %d\n", vec->capacity, vec->capacity * 2);
        exit(1);
    }
    vec->capacity = new_capacity;
    vec->data[vec->size] = c;
...
char CharVec_get(CharVec *vec, int index) {
    if (index >= vec->size || index < 0) {
        printf("Error getting index %d from char vec of size %d\n", index, vec->size);
        exit(1);
    }
    return vec->data[index];
}
...
void CharVec_resize(CharVec *vec, int capacity) {
    if (capacity == 0) {
        printf("Error cant resize vec to 0(use reset)\n");
        exit(1);
    }
    vec->data = (char *)realloc(vec->data, capacity * sizeof(char));
    if (vec->data == NULL) {
        printf("Error resizing vec\n");
        exit(1);
    }
...
void CharVec_set(CharVec *vec, int index, char c) {
    if (index > vec->size || index < 0) {
        printf("Error setting index %d from char vec of size %d\n", index, vec->size);
        exit(1);
    }
    if (index == vec->size) {
        CharVec_push(vec, c);
...
StateMachine StateVec_get(StateVec *vec, int index) {
    if (index >= vec->size || index < 0) {
        printf("Error getting index %d from StateMachine vec of size %d\n", index, vec->size);
        exit(1);
    }
    return vec->data[index];
}
...
void StateVec_set(StateVec *vec, int index, StateMachine m) {
    if (index > vec->size || index < 0) {
        printf("Error setting index %d from StateMachine vec of size %d\n", index, vec->size);
        exit(1);
    }
    if (index == vec->size) {
        StateVec_push(vec, m);
